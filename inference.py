...
# To run script, type into terminal: python inference.py x_test.npy
...
if __name__ == '__main__':
    ...
    correct_count = 0
    # define how many rows (ig?) from the test set to send to the MCU
    test_len = 20
    ...
    for x, y in zip(x_test[:test_len], y_test[:test_len]):
        ...
        num_elements_per_sample = np.prod(sample_shape)
        dtype_size = x_test.dtype.itemsize
        bytes_per_sample = num_elements_per_sample * dtype_size

        num_pred += 1
        class_idx = y
        ser.write(x.tobytes())
        time.sleep(1)
        img = ser.read(bytes_per_sample)
        img = np.frombuffer(img, dtype=np.uint8)
        print("Sample sent to the MCUs: \n {}".format(img))
        time.sleep(1)
        pred = ser.read(10)
        ...
        image = image.resize(sample_shape)  # Adjust size as needed
